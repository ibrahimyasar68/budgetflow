...
- **Play üretim:** **12 (1.5.1) yayında**, %100. **13 (1.6.0) incelemede**, %20 aşamalı sunumla gönderildi.
- **Sürüm tek kaynağı:** `pubspec.yaml` → **`1.6.0+13`**. Play'deki en yüksek versionCode 13 → sıradaki yükleme **14**.
- **AAB:** `build/app/outputs/bundle/release/app-release.aab` — 1.6.0 / 13, 52,9 MB, yüklendi.
- **Kaynak kodu:** https://github.com/ibrahimyasar68/budgetflow (**public**), son commit `9459a2b`.
...
1. **1.6.0 onayını bekle**, sonra izle: Play vitals (çökme/ANR — reklam SDK'sı yeni değişken) ve AdMob doldurma oranı. Bir gün %20'de bekle, temizse **%50 → %100** (Üretim → Sürümler → Sürümü yönet → Kullanıma sunmayı güncelle).
...
3. **Test cihazı kimliği** — `AdService.testDeviceIds` **boş**. Gerçek telefonda release derlemesi denenecekse doldurulmalı; kimlik logcat'te `setTestDeviceIds` satırında. Emülatörler otomatik test cihazı, gerekmiyor. **Kendi reklamına tıklamak AdMob hesabını kapattırabilir.**
...
- Play'in "uçtan uca ekran" uyarıları gerçek kusur değil — Android 16 emülatöründe kontrol edildi; uyarı Flutter motorunun deprecated API kullanımından geliyor.
- "Kod gösterme dosyası yok" uyarısı geçersiz: `build.gradle`'da `minifyEnabled false`, karartma yok.
...
- `play-uretim-surum-notu.md` — üretim sürüm notları (`<tr-TR>`), 1.5.0 / 1.5.1 / 1.6.0 bölümleri.
- `surum-notlari-1.5.0.md`, `surum-notlari-1.5.1.md`, `surum-notlari-1.6.0.md` — teknik notlar. AdMob kimlikleri 1.6.0'ın §8'inde.
